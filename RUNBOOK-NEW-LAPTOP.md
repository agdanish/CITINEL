...
2b. **Manual Deploy citinel-web once, in the morning, within 6 hours of going on stage.** INC-0419 is the demo record whose CERT-In clock counts down live, and its 6-hour window starts at container boot. Without a morning redeploy the dial reads BREACHED. Do this even with Auto-Deploy off; Manual Deploy is a separate button.
2c. Optional, about 20 minutes: renew the Slack connection, and do it WITHIN FOUR HOURS of going on stage. The Slack credential never expires; the login session in auth.json does, after four hours, and nothing refreshes it, so an upload made the night before is dead by morning. The ticketing half already works on Render; only Slack is missing, and the ledger reports the gap honestly rather than claiming a message was sent. Steps are in Claude's demo-morning memory, or ask Claude for "the Slack renewal steps". The one step people get wrong: when macOS offers the Keychain during `swy auth connect Slack`, click **Cancel**, because only a file-based key can be read by the Linux container.
