...
2. **The dashboard is still not wired to live data.** 11 of 18 pages could be, honestly, with real (not fictional) content, but that work was not finished — see §15. This is now the single highest-value remaining work, since Step 7 gives several previously-`NOT_WIREABLE_TODAY` pages (Replay, Confidence) something real to show for the first time.
...
4. **Lyzr Studio agent not yet built** — the connector code and wire format are both fixed and ready (§15), but nothing calls it live until Danish builds the actual agent in Studio with the instructions `lyzr.py`'s docstring now contains.
