...
### Entry screen: removed 4 Sep 2026
`Entry.dc.html` was deleted on Danish's instruction; `/` now redirects to `Overview.dc.html`. The left rail in `nav.js` is the persistent navigation; the per-page top nav is kept alongside it deliberately.
