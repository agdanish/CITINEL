...
3. **Both priority findings are now fixed (2–3 Sep 2026, follow-up commit).** Citation-gating — CITINEL's headline "every claim cites the exact log line" feature — previously only ran on the Narrator's claims; the Correlator's kill-chain stages and the Marshal's proposed actions now go through the identical gate, dropped and disclosed the same way an uncited claim already was. [SEC-F08] Separately, the Triage Router's own AUTO_CLOSE conclusion can no longer silently end an investigation on a high- or critical-severity incident: a deterministic floor, keyed to `incident.severity` (set once from Sigma rule levels before any model sees the case, not attacker-influenceable), forces escalation regardless of what the Router concluded or why — the router's real answer stays on the ledger unedited, and a second entry records the override. Below that floor (low/medium/informational severity), the Router's own conclusion is still trusted, an accepted residual stated in the code. [SEC-F06] Neither fix claims to make the model unpersuadable — SAFE-F02's "mitigates, never solves" discipline still applies to the citation gate itself (a passing citation proves the quote is real, not that it is trustworthy) — but SEC-F06's floor is a genuine, model-independent guarantee for the severity band that matters, and SEC-F08 closes an inconsistency, not merely a gap.
...
| SEC-F06 | High | llm | **FIXED** | An attacker-written log line can steer the Triage Router to `auto_close`, ending the investigation with a reassuring banner |
...
| SEC-F08 | High | llm | **FIXED** | Only the Narrator's claims are mechanically citation-verified; Correlator kill-chain citations and Marshal proposal citations reach the console unchecked |
...
4. **A precise line distinguishing citation-gating from "AI explainability":** most competitors' "explainability" is a confidence score or a natural-language rationale; CITINEL's citation gate is a *mechanical, non-prompted check* that the model's claim text appears verbatim in the evidence it cites — closer to a compiler check than to an LLM's own self-report. (SEC-F08 is now closed: the gate runs identically on Narrator claims, Correlator kill-chain stages, and Marshal proposed actions, so this claim can be stated without the earlier Narrator-only caveat. It still cannot be stated as "proves the claim is true" — only that the quote is real; see SAFE-F02's existing discipline.)
...
3. ~~Extend citation-gate enforcement to Correlator and Marshal claims~~ **done** (SEC-F08).
4. ~~Harden the Triage Router against an attacker-authored `auto_close`~~ **done** (SEC-F06, severity floor). Fence the Marshal's unfenced interpolation of model-written text (SEC-F07) remains open — a different, not-yet-fixed injection finding.
